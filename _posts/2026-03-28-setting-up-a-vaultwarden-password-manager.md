...
excerpt: "Password vaults are a convenient and secure way to manage multiple passwords. As data breaches become more and more common, password complexity and even rules for regularly updating them leads to an inevitable mishmash of credentials that are impossible to remember when not used daily. The logic behind constantly-evolving password guidelines is beyond the scope of this guide, but recent word from NIST on password managers recommends their use.
...
Some downsides exist, though; it's a resource hog when hosted locally, and resources matter when you keep stacking services on your home Debian device. The average user does not need the scalability and infrastructure that it provides. Additionally, some services such as autofilling passwords require you pay a subscription.
...
On the server meant to host the Vaultwarden instance, install `podman` (you could use docker if you like, but I do not enjoy everything having root access) and set up the necessary folder structure in your home directory. You will also need to set up a self-signing certificate (acceptable since we are on a VPN).
...
```
The certificate expires in 100 years (36,500 days) because we can't disable expiration completely.
